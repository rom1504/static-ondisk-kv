...
def test_hello_world(tmp_path):
    ...
    df.to_parquet(folder + "/my_file.parquet")

    sort_parquet(
        input_collection=folder + "/my_file.parquet",
        key_column="hash",
        value_columns=["pwatermark", "punsafe"],
        output_folder=folder + "/unsafe_collection",
    )

    parquet_to_file(
        input_collection=folder + "/unsafe_collection",
        key_column="hash",
        value_columns=["pwatermark", "punsafe"],
        output_file=folder + "/ordered_file",
        key_format="q",
        value_format="ee",
    )
    kv = OnDiskKV(file=folder + "/ordered_file", key_format="q", value_format="ee")
    ...
    r = [random.randint(0, kv.length) for _ in range(10**2)]
    for i in tqdm(r):
        k = kv.get_key(i)
        kv[k]
